...
> [!WARNING]
> BLE Improv is temporarily disabled due to performance issues.
...
> [!WARNING]
> This feature is temporarily disabled due to performance issues. We’re actively working on stabilizing it.
...
> [!IMPORTANT]
> Make sure your Wi-Fi is 2.4 GHz — 5 GHz is not supported.
...
> [!TIP]
> Let the sensor run for a few minutes before calibrating for the most stable results.
...
> [!TIP]
> Only perform this when you're confident the air is clean and stable. (The BME688 calibrates itself automatically — no user action required.)
...
> [!TIP]
> Use Ctrl+F (Windows) or Cmd+F (Mac) to quickly find and replace all instances.

> [!NOTE]
> Seeing "Visual editor not supported"? That’s normal for custom cards — you can safely ignore it.
...
> [!NOTE]
> A new and improved Zone Editor Add-on for Home Assistant is now available!
...
> [!NOTE]
> Make sure your desktop is connected to the same network as your Home Assistant / Sensor.

> [!TIP]
> If you run into trouble, you can always have a look at the [YouTube video](https://www.youtube.com/watch?v=raLACrPG8EM) for guidance.
...
> [!TIP]
> If the page times out during upload, just refresh the page or try again.
...
> [!IMPORTANT]
> Only the bottom-facing USB-C port supports data transfer.
